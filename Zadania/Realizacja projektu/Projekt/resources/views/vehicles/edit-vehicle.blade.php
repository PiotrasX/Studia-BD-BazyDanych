...
@include('shared.head', ['pageTitle' => 'Edytuj pojazd'])
    ...
    <main>
        <div class="container my-3">
            <div class="row mb-4 text-center">
                <h1>Edycja pojazdu {{ $pojazd->cechyPojazdu->marka }} {{ $pojazd->cechyPojazdu->model }}</h1>
            </div>
            @include('shared.validation-error')
            <div class="row d-flex justify-content-center">
                <div class="col-10 col-sm-10 col-md-8 col-lg-6 col-xl-5 col-xxl-4">
                    <form method="POST" action="{{ route('vehicles.edit.editValidate') }}" class="needs-validation"
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="btn-group btn-group-toggle input-group mb-3" data-toggle="buttons">
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('marka_input_type', $selectedInputTypeMarka) === 'text' ? 'active' : '' }}">
                                    <input type="radio" name="marka_input_type" id="marka_text" value="text"
                                        {{ old('marka_input_type', $selectedInputTypeMarka) === 'text' ? 'checked' : '' }}>
                                    Wpisz
                                </label>
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('marka_input_type', $selectedInputTypeMarka) === 'select' ? 'active' : '' }}">
                                    <input type="radio" name="marka_input_type" id="marka_select" value="select"
                                        {{ old('marka_input_type', $selectedInputTypeMarka) === 'select' ? 'checked' : '' }}>
                                    Wybierz
                                </label>
                            </div>
                            <input id="marka_input" name="marka" type="text"
                                class="form-control {{ $errors->has('marka') ? 'is-invalid' : '' }}"
                                value="{{ old('marka', $pojazd->cechyPojazdu->marka) }}"
                                style="{{ old('marka_input_type', $selectedInputTypeMarka) === 'select' ? 'display: none;' : 'display: block;' }}">
                            <select id="marka_select_option" name="marka_select"
                                class="form-select form-control {{ $errors->has('marka_select') ? 'is-invalid' : '' }}"
                                style="{{ old('marka_input_type', $selectedInputTypeMarka) === 'text' ? 'display: none;' : 'display: block;' }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="btn-group btn-group-toggle input-group mb-3" data-toggle="buttons">
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('model_input_type', $selectedInputTypeModel) === 'text' ? 'active' : '' }}">
                                    <input type="radio" name="model_input_type" id="model_text" value="text"
                                        {{ old('model_input_type', $selectedInputTypeModel) === 'text' ? 'checked' : '' }}>
                                    Wpisz
                                </label>
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('model_input_type', $selectedInputTypeModel) === 'select' ? 'active' : '' }}">
                                    <input type="radio" name="model_input_type" id="model_select" value="select"
                                        {{ old('model_input_type', $selectedInputTypeModel) === 'select' ? 'checked' : '' }}>
                                    Wybierz
                                </label>
                            </div>
                            <input id="model_input" name="model" type="text"
                                class="form-control {{ $errors->has('model') ? 'is-invalid' : '' }}"
                                value="{{ old('model', $pojazd->cechyPojazdu->model) }}"
                                style="{{ old('model_input_type', $selectedInputTypeModel) === 'select' ? 'display: none;' : 'display: block;' }}">
                            <select id="model_select_option" name="model_select"
                                class="form-select form-control {{ $errors->has('model_select') ? 'is-invalid' : '' }}"
                                style="{{ old('model_input_type', $selectedInputTypeModel) === 'text' ? 'display: none;' : 'display: block;' }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="btn-group btn-group-toggle input-group mb-3" data-toggle="buttons">
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'text' ? 'active' : '' }}">
                                    <input type="radio" name="nadwozie_input_type" id="nadwozie_text" value="text"
                                        {{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'text' ? 'checked' : '' }}>
                                    Wpisz
                                </label>
                                <label
                                    class="btn btn-secondary custom-toggle {{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'select' ? 'active' : '' }}">
                                    <input type="radio" name="nadwozie_input_type" id="nadwozie_select" value="select"
                                        {{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'select' ? 'checked' : '' }}>
                                    Wybierz
                                </label>
                            </div>
                            <input id="nadwozie_input" name="nadwozie" type="text"
                                class="form-control {{ $errors->has('nadwozie') ? 'is-invalid' : '' }}"
                                value="{{ old('nadwozie', $pojazd->cechyPojazdu->nadwozie) }}"
                                style="{{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'select' ? 'display: none;' : 'display: block;' }}">
                            <select id="nadwozie_select_option" name="nadwozie_select"
                                ...
                                style="{{ old('nadwozie_input_type', $selectedInputTypeNadwozie) === 'text' ? 'display: none;' : 'display: block;' }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <input id="rok_produkcji" name="rok_produkcji" type="text"
                                ...
                                value="{{ old('rok_produkcji', $pojazd->rok_produkcji) }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="input-group mb-2">
                                <input id="przebieg" name="przebieg" type="text"
                                    ...
                                    value="{{ old('przebieg', $pojazd->przebieg) }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="input-group mb-2">
                                <input id="pojemnosc_silnika" name="pojemnosc_silnika" type="text"
                                    ...
                                    value="{{ old('pojemnosc_silnika', $pojazd->pojemnosc_silnika) }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="input-group mb-2">
                                <input id="moc_silnika" name="moc_silnika" type="text"
                                    ...
                                    value="{{ old('moc_silnika', $pojazd->moc_silnika) }}">
                                <span class="input-group-text">KM </span>
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <select id="rodzaj_paliwa" name="rodzaj_paliwa"
                                ...
                                <option value="benzyna"
                                    {{ old('rodzaj_paliwa', $pojazd->rodzaj_paliwa) === 'benzyna' ? 'selected' : '' }}>
                                    benzyna</option>
                                <option value="benzyna + gaz"
                                    {{ old('rodzaj_paliwa', $pojazd->rodzaj_paliwa) === 'benzyna + gaz' ? 'selected' : '' }}>
                                    benzyna + gaz</option>
                                <option value="diesel"
                                    {{ old('rodzaj_paliwa', $pojazd->rodzaj_paliwa) === 'diesel' ? 'selected' : '' }}>
                                    diesel</option>
                                <option value="diesel + gaz"
                                    {{ old('rodzaj_paliwa', $pojazd->rodzaj_paliwa) === 'diesel + gaz' ? 'selected' : '' }}>
                                    diesel + gaz</option>
                            </select>
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <input id="liczba_drzwi" name="liczba_drzwi" type="text"
                                ...
                                value="{{ old('liczba_drzwi', $pojazd->liczba_drzwi) }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <input id="liczba_miejsc" name="liczba_miejsc" type="text"
                                ...
                                value="{{ old('liczba_miejsc', $pojazd->liczba_miejsc) }}">
                        ...
                        <div class="form-group mb-3 card card-p-bg-yellow">
                            ...
                            <div class="zdjecia_istniejace">
                                @foreach ($pojazd->zdjecia as $zdjecie)
                                    <div class="image-entry mb-2">
                                        <img src="{{ asset('storage/img/vehicles/' . $zdjecie->nazwa_zdjecia) }}"
                                            alt="Zdjęcie pojazdu" class="img-thumbnail col-8 mb-1">
                                        <input class="mb-3" type="checkbox" name="zdjecia_pojazdow_usun[]"
                                            value="{{ $zdjecie->id_zdjecia }}"
                                            {{ in_array($zdjecie->id_zdjecia, session('zdjeciaPojazdowUsun', [])) ? 'checked' : '' }}>
                                        Usuń
                                        </label>
                                    </div>
                                @endforeach
                        ...
                        <div class="form-group">
                            <input type="hidden" name="IDpojazdu" value="{{ $pojazd->id_pojazdu }}">
                        </div>
